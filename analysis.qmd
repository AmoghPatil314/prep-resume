...
library(tidybayes)
library(brms)
resumes_data<-read.csv(file="./data/resume.csv")
```
...
# Fit the logistic regression model using brm
fit <- resumes_data|>brm(formula = formula, family = bernoulli(), refresh= 0, silent=2, seed=9)
...
#| cache: true
fit|>
  add_epred_draws(newdata=resumes_data)|>
    #select(.epred)|>
      ggplot(aes(x=.epred,fill=race))+
      geom_histogram(alpha=1,position="identity",bins=112)+
